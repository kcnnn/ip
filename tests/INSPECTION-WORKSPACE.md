...
# Photo-led observations

The notebook accepts a camera photo, upload, or saved original before dictation. Analyze sends the image and transcript together; extracted inspector fields and AI visual findings remain separate. Add to report saves both with the photo link. Editing the transcript/photo clears the prior review. Text-only notes remain available.

`tests/photo-observation.browser.cjs` checks original capture/storage, a real multimodal request shape with mocked provider output, report rendering, transcript-change invalidation, API errors, and mobile overflow. These tests do not establish live model accuracy or credential validity. Test real inspection photos on the deployed site with the configured provider before field use.
