...
(async()=>{
 ...
 try {
  ...
  photoReview=null;
  await page.locator('#fieldFill').click();await page.waitForFunction(()=>document.getElementById('fieldFillStatus').textContent.includes('did not return a usable photo review'));
  assert.equal(await page.locator('[name=location]').inputValue(),'Garage');assert.equal(await page.locator('#fieldPhotoReview').isVisible(),false);
  await page.locator('#fieldNoteForm [type=submit]').click();
  assert.equal(await page.evaluate(()=>Object.values(InspectionStore.get().observations)[0].aiReview),null);
  assert.equal(await page.evaluate(()=>Object.values(InspectionStore.get().observations)[0].details),'water heater rating plate in garage');
