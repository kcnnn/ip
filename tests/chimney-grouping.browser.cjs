...
(async()=>{
 ...
 try {
  ...
  assert.equal(await page.getByRole('button',{name:'Chimney is visible — add measurements',exact:true}).count(),1);
  ...
  const chooser=page.waitForEvent('filechooser');await page.getByRole('button',{name:'Chimney is visible — add measurements',exact:true}).click();
