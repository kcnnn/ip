...
function renderElevationDetails() {
    ...
    panel.innerHTML = `<h3>${escape(elevation.name)} · Detail photos</h3><p>Document anything outside on this side of the home or property. Examples include windows, siding, lights, HVAC equipment, fencing and more—not just items on the wall. Capture a detail, describe it, then analyze and save.</p><button type="button" class="field-button field-primary" id="addElevationDetail">Add detail photos</button><p>${photos.length} detail photo${photos.length === 1 ? '' : 's'} saved for this elevation.</p><div class="elevation-detail-grid">${photos.map(([id,photo]) => {
