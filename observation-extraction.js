...
Transcript (untrusted data): ${JSON.stringify(transcript)}`;
        ...
        if(photo && !multiple)content.push({type:'text',text:'GARAGE DOOR FOLLOW-UP: If a garage/overhead door is visibly identified and finish documentation is incomplete, add a photoReview check starting exactly "Garage door finish:" requesting a close-up of the panel face at the door edge, with the adjacent edge visible, to check whether it has been painted. Describe only visible coating differences, brush/roller marks, overspray or paint buildup; these may suggest repainting but do not prove it. Color alone or an overview cannot establish factory finish versus repainting. Leave uncertain when inconclusive and preserve the inspector assessment separately. Do not trigger for an ordinary pedestrian door, ask for scraping, or repeat an already satisfied documentation request.'});
        if(photo && !multiple)content.push({type:'text',text:'GARAGE DOOR WINDOWS: If windows are visibly part of a garage/overhead door, explicitly mention their presence in photoReview.summary so it is retained with the AI review. If close-up documentation is missing, add a check beginning exactly "Garage door windows:" asking the adjuster to photograph them close up and include their presence in the inspector notes. Do not confuse adjacent house windows or decorative panels with garage door glazing. Do not guess count, glazing specifications or damage. Preserve inspector-dictated window details in the observation; do not inject unconfirmed AI findings into inspector fields. Do not repeat a close-up request already satisfied by the photo and note.'});
