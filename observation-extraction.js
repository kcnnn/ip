...
const ObservationExtraction = {
    ...
    purposeGuidance: {
        ...
        brittle:'BRITTLE TEST: Document before/after condition and inspector-reported handling. A still image cannot prove flexibility, test pass, or whole-roof repairability. Preserve existing damage separately from changes during the test.',
...
Transcript (untrusted data): ${JSON.stringify(transcript)}`;
        ...
        if (photo) {
            ...
            content.unshift({type:'text',text:`Selected photo (${['before','after'].includes(photoContext.brittlePhase)?photoContext.brittlePhase+' test':'primary view'}):`}, { type: 'image', source: { type: 'base64', media_type: mediaType, data: base64Data } });
            ...
            for(const item of comparisons) {
                if(!['before','after'].includes(item.phase))throw new Error('Comparison photo needs a before/after label.');
            ...
            if(comparisons.length) content.push({type:'text',text:'PAIRED REVIEW: Compare the labeled before and after photos together. Describe what was already present versus what visibly changed. Identify the phase supporting each finding. Account for lighting, hand position and viewpoint differences; do not invent a change where coverage differs. State when the same area cannot be compared. Manual lifting is not wind damage. A visual change during testing does not establish original storm damage. Keep test pass/fail attributed to the inspector; do not invent it from photos.'});
